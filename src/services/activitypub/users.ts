...
export async function user(user: ITwitterUserProfile) {
    ...
    attachments.push(utils.jsonld.createAttachment('お知らせ', `このユーザーはTwitterAPIを通して生成されたユーザーであり、現在閲覧されているインスタンスに帰属するユーザーではありません。このユーザーに関するお問い合わせは <a href="${config.domain}">${config.domain}</a> へお願いいたします。`))
