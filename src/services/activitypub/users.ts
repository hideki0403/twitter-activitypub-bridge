...
export async function user(user: ITwitterUserProfile) {
    ...
    if (user.url) {
        const userLink = await twitter.replaceRawLinks(user.url, user.entities)
        attachments.push(utils.jsonld.createAttachment('Link', `<a href="${userLink}">${userLink}</a>`))
    }
    ...
    attachments.push(utils.jsonld.createAttachment('お知らせ', `このユーザーはTwitterAPIを通して生成されたユーザーであり、現在閲覧されているインスタンス(サーバー)に帰属するユーザーではありません。このユーザーに関するお問い合わせは <a href="${config.url}">${config.domain}</a> へお願いいたします。`))

    const summary = utils.text.entities2html(await twitter.replaceRawLinks(user.description, user.entities))
